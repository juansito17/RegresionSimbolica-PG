...
class GeneticAlgorithm {
    struct Island {
        ...
        int id;
        // --- Meta-evolución: historial de parámetros y desempeño ---
        struct ParamHistoryEntry {
            EvolutionParameters params;
            double fitness;
        };
        std::vector<ParamHistoryEntry> param_history; // Historial de parámetros y fitness

        explicit Island(int island_id, int pop_size) : id(island_id) {
             population = create_initial_population(pop_size);
             params = EvolutionParameters::create_default();
        }
        // Registrar desempeño de parámetros actuales
        void record_param_performance(double fitness) {
            param_history.push_back({params, fitness});
        }
    };
    ...
    PatternMemory global_pattern_memory; // Memoria evolutiva global compartida
