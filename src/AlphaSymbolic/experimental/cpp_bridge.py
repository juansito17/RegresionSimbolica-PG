...
class GPEngine:
    def __init__(self, binary_path=None):
        if binary_path is None:
            # Default location: .local/build/cpp_legacy/Release/SymbolicRegressionGP.exe
            # Assuming we are in WarpSymbolic/.. root or similar.
            # Adjust path relative to this file: alphasybolic/core/gp_bridge.py
            # The legacy engine is kept separate from the production backend.
            # Improved Project Root Detection
            # Walk upward looking for legacy/cpp_engine.
            ...
            d = current_dir
            for _ in range(5):
                if os.path.exists(os.path.join(d, "legacy", "cpp_engine")):
                    project_root = d
                    break
            ...
            if is_windows:
                 search_paths = [
                    os.path.join(base_dir, ".local", "build", "cpp_legacy", "Release", "SymbolicRegressionGP.exe"),
                    os.path.join(base_dir, ".local", "build", "cpp_legacy", "SymbolicRegressionGP.exe"),
                    os.path.join(base_dir, "build", "cpp_legacy", "Release", "SymbolicRegressionGP.exe"),
                    os.path.join(base_dir, "build", "cpp_legacy", "SymbolicRegressionGP.exe"),
                    os.path.join(base_dir, "legacy", "cpp_engine", "build", "Release", "SymbolicRegressionGP.exe"),
                    # Fallbacks
                     os.path.join(base_dir, "legacy", "cpp_engine", "build", "Release", "SymbolicRegressionGP"),
                 ]
            else:
                 # Linux/Mac (Colab) - Prioritize no extension
                 search_paths = [
                    os.path.join(base_dir, ".local", "build", "cpp_legacy", "SymbolicRegressionGP"),
                    os.path.join(base_dir, ".local", "build", "cpp_legacy", "Release", "SymbolicRegressionGP"),
                    os.path.join(base_dir, "build", "cpp_legacy", "SymbolicRegressionGP"),
                    os.path.join(base_dir, "build", "cpp_legacy", "Release", "SymbolicRegressionGP"),
                    os.path.join(base_dir, "legacy", "cpp_engine", "build", "SymbolicRegressionGP"),
                    os.path.join(base_dir, "legacy", "cpp_engine", "build", "Release", "SymbolicRegressionGP"),
                    # Fallbacks
                    os.path.join(base_dir, "legacy", "cpp_engine", "build", "Release", "SymbolicRegressionGP.exe"),
                    os.path.join(base_dir, "legacy", "cpp_engine", "build", "SymbolicRegressionGP.exe"),
                 ]
