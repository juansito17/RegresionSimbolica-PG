"""
Beam Search for WarpSymbolic.
Explores multiple formula candidates in parallel, keeping top-K at each step.
"""
import torch
import numpy as np
from warpsymbolic.symbolic.grammar import VOCABULARY, OPERATORS, TOKEN_TO_ID, ExpressionTree, OPERATOR_STAGES
...
class BeamSearch:
    def __init__(self, model, device, beam_width=10, max_length=30, curriculum_stage=None, num_variables=1):
        ...
        from warpsymbolic.symbolic.grammar import VARIABLES
        if num_variables == 1:
...
if __name__ == "__main__":
    ...
    model = WarpSymbolicModel(vocab_size=VOCAB_SIZE + 1, d_model=64).to(DEVICE)
    try:
