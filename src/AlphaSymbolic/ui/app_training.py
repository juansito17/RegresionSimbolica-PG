"""
Training functions for WarpSymbolic Gradio App.
...
from warpsymbolic.symbolic.grammar import VOCABULARY, TOKEN_TO_ID, OPERATORS, OPERATOR_STAGES, VARIABLES
from AlphaSymbolic.data.synthetic_data import DataGenerator
from AlphaSymbolic.ui.app_core import (
    UI_RESULTS_DIR,
    get_model,
    save_model,
    TRAINING_STATUS,
...
from AlphaSymbolic.experimental.neural.loss import QuantileLoss
from AlphaSymbolic.experimental.search.hybrid_search import hybrid_solve
from warpsymbolic.symbolic.grammar import ExpressionTree, simplify_formula
from AlphaSymbolic.data.data_utils import normalize_batch
    ...
    try:
        ...
        for iteration in range(int(iterations)):
            ...
            try:
                ...
                from AlphaSymbolic.experimental.search.beam_search import BeamSearch
                    ...
                    if res and res.get('formula') and gp_rmse <= 0.05:
                        try:
                            ...
                            try:
                                ...
                                log_path = os.path.join(UI_RESULTS_DIR, "learned_formulas.csv")
...
def train_from_memory(epochs=10, batch_size=32, num_variables=1, progress=gr.Progress()):
    ...
    log_file = os.path.join(UI_RESULTS_DIR, "learned_formulas.csv")
