"""
Core state and model management for WarpSymbolic Gradio App.
"""
import torch
import os
from AlphaSymbolic.experimental.neural.model import WarpSymbolicModel
from warpsymbolic.symbolic.grammar import VOCABULARY
...
MODEL_DIR = os.path.join(_REPO_ROOT, "research", "models")
UI_RESULTS_DIR = os.path.join(_REPO_ROOT, "benchmarks", "raw", "ui")
...
def load_model(force_reload=False, preset_name=None):
    ...
    drive_path = "/content/drive/MyDrive/WarpSymbolic_Models"
...
def save_model():
    ...
    if MODEL is not None:
        filename = f"warp_symbolic_model_{CURRENT_PRESET}.pth"
        ...
        if os.path.exists("/content/drive"):
            ...
            try:
                ...
                FILES_TO_BACKUP = [
                    (os.path.join(UI_RESULTS_DIR, 'learned_formulas.csv'), 'learned_formulas.csv')
                ]
