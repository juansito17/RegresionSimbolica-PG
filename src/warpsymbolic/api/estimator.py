...
from warpsymbolic.gpu.errors import GpuUnavailableError
...
class WarpSymbolicRegressor(RegressorMixin, BaseEstimator):
    ...
    def __init__(
        ...
        *,
        ...
        # CUDA is the production default. CPU remains an explicit opt-in for
        # parser, validation and debugging workflows.
    ...
    def _validate_parameters(self) -> None:
        ...
        if device_name != "cpu" and not device_name.startswith("cuda"):
            raise ValueError("device must be 'cuda', 'cuda:<index>', or explicit 'cpu' debug mode")

    def _require_cuda(self) -> None:
        """Reject silent CPU fallback for the production estimator path."""
        ...
        if not torch.cuda.is_available():
            raise GpuUnavailableError(
                "WarpSymbolic requires a CUDA-capable GPU for production fitting. "
                "Use device='cpu' only for explicit tests or debugging."
            )
    ...
    def _coerce_fit_data(self, X: Any, y: Any):
        ...
        if target.ndim == 2 and target.shape[1] == 1:
            target = target[:, 0]
        if target.ndim != 1:
            raise ValueError("WarpSymbolicRegressor supports one target only")
