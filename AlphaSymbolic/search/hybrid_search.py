...
def hybrid_solve(
    ...
    if neural_results:
        ...
        if len(seeds) > 0:
            # Print the one(s) we actually picked from NN
            # If we appended to existing seeds (extra_seeds), we need to look at the end
            # We know we just added 'selected_seeds' or 'all_candidates'
            newly_added_count = len(seeds) - (len(extra_seeds) if extra_seeds else 0)
            # Just print the last added one as "Random Seed NN"
            print(f"Random Seed NN: {seeds[-1]}") 
    else:
