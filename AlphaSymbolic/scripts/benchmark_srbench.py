...
DEFAULT_CACHE = Path("cache/srbench_2025")
DEFAULT_OUTPUT = Path("benchmarks/srbench_2025.jsonl")
DEFAULT_RUNNER = "AlphaSymbolic.scripts.srbench_runner:run"
    ...
    try:
        if opener is None:
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "AlphaSymbolic-SRBench/1.0"},
            )
...
def _git_metadata() -> Dict[str, Any]:
    ...
    if isinstance(listed, bytes):
        for raw_path in sorted(path for path in listed.split(b"\0") if path):
            ...
            source_file_count += 1
    return {
        "alphasymbolic_commit": commit,
        "alphasymbolic_tree_dirty": status not in {"", "unknown"},
        "alphasymbolic_diff_sha256": (
        ...
        "alphasymbolic_source_sha256": (
            source_digest.hexdigest() if source_file_count else None
        ),
        "alphasymbolic_source_file_count": source_file_count,
    }
...
def _make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--algorithm", default="AlphaSymbolic")
    parser.add_argument(
        "--track",
