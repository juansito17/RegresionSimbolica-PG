...
def evaluate_gen8_new(n):
    x0 = float(n)
    x1 = float(n % 6)
    x2 = float(n % 2)
    
    # Formula (0.057): 
    # ((lgamma((x0 + 2)) - cos((x1 / ((0.11580232 * x0) ^ (((sin(-0.16773652) + (x0 / sin(gamma(x2)))) + x0) + x2))))) - x0)
    ...
    try:
        ...
        # (((sin(-0.16773652) + (x0 / sin_g)) + x0) + x2)
        term_sin_const = math.sin(-0.16773652)
        term_div = x0 / sin_g
        
        exponent = term_sin_const + term_div + x0 + x2
...
with open("eval_gen8_new.txt", "w") as f:
