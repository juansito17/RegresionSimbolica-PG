...
def tree_structural_distance(a: Optional[Node], b: Optional[Node]) -> float:
    """Calcula la distancia estructural entre dos árboles (nodos)."""
    ...
    if a.type == NodeType.CONSTANT:
        return min(1.0, abs(a.value - b.value) / 10.0)
    if a.type == NodeType.VARIABLE:
        return 0.0
    if a.type == NodeType.OPERATOR:
        op_dist = 0.0 if a.op == b.op else 1.0
        left_dist = tree_structural_distance(a.left, b.left)
        right_dist = tree_structural_distance(a.right, b.right)
        return (op_dist + left_dist + right_dist) / 3.0
    return 1.0
