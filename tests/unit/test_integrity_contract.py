...
def test_production_runner_has_no_dataset_identity_branch():
    source = (ROOT / "src" / "warpsymbolic" / "cli" / "srbench_runner.py").read_text(
        encoding="utf-8"
    ).lower()
...
def test_engine_core_has_no_embedded_evaluation_sequence():
    core = ROOT / "src" / "warpsymbolic" / "gpu"
    source = "\n".join(
...
def test_runner_parameters_ignore_dataset_group_at_execution():
    from warpsymbolic.cli.srbench_runner import _effective_params
