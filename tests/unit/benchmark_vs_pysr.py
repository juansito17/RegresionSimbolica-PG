"""
╔══════════════════════════════════════════════════════════════════╗
║       WarpSymbolic vs PySR — Benchmark Comparativo            ║
║                                                                ║
║  Compara el motor GPU (TensorGeneticEngine) de WarpSymbolic   ║
...
Uso:
    ...
    python benchmark_vs_pysr.py --only alpha      # Solo WarpSymbolic
"""
...
# ─────────────────────────────────────────────────────────────────
# Runner de WarpSymbolic (GPU Engine)
# ─────────────────────────────────────────────────────────────────
...
def run_alphasybolic(problem: BenchmarkProblem, timeout_sec: int = 30, use_sniper: bool = True, use_structural_seeds: bool = True) -> MethodResult:
    """Ejecuta WarpSymbolic en un problema — single run con todo el tiempo disponible + BFGS."""
    import torch
...
def _eval_formula_metrics(formula: str, x: np.ndarray, y: np.ndarray) -> Tuple[float, float]:
    ...
    safe_dict = {
        ...
        'neg': lambda x: -x,  # Soporte para neg() de WarpSymbolic
    }
...
def run_benchmark(
    ...
    if run_alpha:
        if use_sniper:
            alpha_label = "WarpSymbolic"
        elif use_structural_seeds:
            alpha_label = "WarpSymbolic (Sin Sniper)"
        else:
            alpha_label = "WarpSymbolic (Pure GP)"
    ...
    for i, problem in enumerate(problems):
        ...
        if run_alpha:
            print(f"  ▸ Ejecutando WarpSymbolic...", end="", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark: WarpSymbolic vs PySR")
