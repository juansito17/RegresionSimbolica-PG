...
def verify_isolation():
    ...
    for i in range(n_islands):
        ...
        if n_bad > 0:
            ...
            # Show some examples and their indices
            bad_indices = torch.where(mask_bad)[0] + start
            bad_vals = island_c[mask_bad]
            for idx, val in zip(bad_indices.tolist(), bad_vals.tolist()):
                print(f"  - Absolute Index: {idx}, Value: {val}")
            errors += n_bad
        else:
