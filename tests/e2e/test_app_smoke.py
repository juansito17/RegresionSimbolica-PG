...
def test_gradio_app_opens_in_browser_if_playwright_available():
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        value for value in (str(src_path), env.get("PYTHONPATH", "")) if value
    )
    proc = subprocess.Popen(
        [sys.executable, "-m", "AlphaSymbolic.app"],
        ...
        with sync_playwright() as pw:
            ...
            for _ in range(60):
                try:
                    ...
                    if "WarpSymbolic" in page.content():
                        break
                except Exception:
                    time.sleep(0.5)
            assert "WarpSymbolic" in page.content()
