...
def test_search_entrypoints_import_without_optional_data_package():
    basic_search = importlib.import_module("AlphaSymbolic.experimental.search.basic_search")
    search_pro = importlib.import_module("AlphaSymbolic.experimental.search.search_pro")
...
def test_basic_search_consumes_mcts_result_dictionary(monkeypatch, tmp_path):
    from AlphaSymbolic.experimental.search import basic_search

    monkeypatch.setattr(
        basic_search, "WarpSymbolicModel", lambda **_kwargs: _FakeModel())
...
def test_pro_search_mcts_flow_survives_missing_pattern_memory(
    ...
    from AlphaSymbolic.experimental.search import search_pro

    monkeypatch.setattr(
        search_pro, "WarpSymbolicModel", lambda **_kwargs: _FakeModel())
